...
def add_planets(sim, planets=["all"], epoch=2459580.5):
    ...
    if len(notplanets) > 0:
        ...
        tsim.add(m=1.0, x=0.0, y=0.0, z=0.0, vx=0.0, vy=0.0, vz=0.0)
...
def initialize_simulation(
    ...
    sflag, epoch, sbx, sby, sbz, sbvx, sbvy, sbvz, weights = (
        horizons_api.query_sb_from_jpl(
            des=des,
            clones=clones,
    ...
    # missing planets
    apflag, sim, sx, sy, sz, svx, svy, svz = add_planets(
...
def initialize_simulation_at_epoch(
    ...
    # get the small body's position and velocity
    flag, sbx, sby, sbz, sbvx, sbvy, sbvz = horizons_api.query_sb_from_horizons(
        des=des, epoch=epoch
    )
    if flag < 1:
        print(
...
def initialize_simulation_from_simarchive(
    des=None, archivefile=None, datadir="", logfile=False
):
    """
    ...
    if logfile:
        time = sim.t
        logmessage += (
            "Loaded integration for " + str(des) + " from " + archivefile + "\n"
        )
