...
def calc_ML_features(
    ...
    delta_length_long = np.abs((time[-1] - time[0]) - 1.0e7)
    ...
    (
        ...
        f.a_stddev_short,
        f.a_delta_short,
        f.a_stddev_normed_short,
        f.a_delta_normed_short,
    ) = basic_time_series_features(a_short)

    f.e_min, f.e_max, f.e_mean, f.e_stddev, f.e_delta, junk, junk = (
        basic_time_series_features(ec)
    )
    (
        f.e_min_short,
        f.e_max_short,
    ...
    f.i_min, f.i_max, f.i_mean, f.i_stddev, f.i_delta, junk, junk = (
        basic_time_series_features(inc)
    )
    (
        f.i_min_short,
        f.i_max_short,
    ...
    (
        ...
        f.q_stddev_short,
        f.q_delta_short,
        f.q_stddev_normed_short,
        f.q_delta_normed_short,
    ) = basic_time_series_features(q_short)

    f.tn_min, f.tn_max, f.tn_mean, f.tn_stddev, f.tn_delta, junk, junk = (
        basic_time_series_features(tn)
    )
    (
        f.tn_min_short,
        f.tn_max_short,
    ...
    # long sims
    f.o_min, f.o_max, argperi_mean, argperi_std, argperi_del, junk, junk = (
        basic_time_series_features(argperi)
    )
    ...
    for n in range(0, clones + 1):
        if (
            argperi_del2[n] < argperi_del[n]
            and np.abs(argperi_del2[n] - argperi_del[n]) > 0.08726
        ):
            f.o_delta[n] = argperi_del2[n]
            f.o_mean[n] = np.mod(argperi_mean2[n], 2.0 * np.pi)
    ...
    (
        ...
        f.Omdot_stddev,
        f.Omdot_delta,
        f.Omdot_stddev_normed,
        f.Omdot_delta_normed,
    ...
    (
        ...
        f.podot_stddev,
        f.podot_delta,
        f.podot_stddev_normed,
        f.podot_delta_normed,
    ...
    (
        ...
        f.Omdot_max_short,
        f.Omdot_mean_short,
        f.Omdot_stddev_short,
        f.Omdot_delta_short,
        f.Omdot_stddev_normed_short,
        f.Omdot_delta_normed_short,
    ) = basic_time_series_features(nodedot)
    (
        f.odot_min_short,
        f.odot_max_short,
        ...
        f.odot_stddev_short,
        f.odot_delta_short,
        f.odot_stddev_normed_short,
        f.odot_delta_normed_short,
    ) = basic_time_series_features(argperidot)
    (
        f.podot_min_short,
        f.podot_max_short,
        f.podot_mean_short,
        f.podot_stddev_short,
        f.podot_delta_short,
        f.podot_stddev_normed_short,
        f.podot_delta_normed_short,
    ) = basic_time_series_features(pomegadot)
    ...
    for n in range(0, clones + 1):
        (
            f.grid_n_empty[n],
            f.nonzero_grid_avg[n],
            f.nonzero_grid_stddev[n],
            f.grid_stddev[n],
            f.grid_delta_stddev[n],
            f.empty_peri_sec[n],
            f.adj_empty_peri_sec[n],
            f.stddev_n_peri_sec[n],
            f.delta_n_peri_sec[n],
            f.rz_peri_max[n],
            f.empty_apo_sec[n],
            f.stddev_apo_sec[n],
            f.delta_n_apo_sec[n],
        ...
        (
            f.grid_n_empty_short[n],
            f.nonzero_grid_avg_short[n],
            f.nonzero_grid_stddev_short[n],
            f.grid_stddev_short[n],
            f.grid_delta_stddev_short[n],
            f.empty_peri_sec_short[n],
            f.adj_empty_peri_sec_short[n],
            f.stddev_n_peri_sec_short[n],
            f.delta_n_peri_sec_short[n],
            f.rz_peri_max_short[n],
            f.empty_apo_sec_short[n],
            f.stddev_apo_sec_short[n],
            f.delta_n_apo_sec_short[n],
            f.rz_apo_max_short[n],
    ...
    for n in range(0, clones + 1):
        ...
        (
            f.a_spectral_fraction_short[n],
            f.a_maxpower_short[n],
            f.a_maxpower3_short[n],
            f.a_frequency1_short[n],
            f.a_frequency2_short[n],
            f.a_frequency3_short[n],
        ) = spectral_characteristics(a_short[n], deltat_short)
        # eccentricity, via e*sin(varpi)
        hec = ec[n] * np.sin(pomega[n])
        (
            f.e_spectral_fraction[n],
            f.e_maxpower[n],
            f.e_maxpower3[n],
            f.e_frequency1[n],
            f.e_frequency2[n],
        ...
        # amd
        amd = 1.0 - np.sqrt(1.0 - ec[n] * ec[n]) * np.cos(inc[n])
        amd = amd * np.sqrt(a[n])
        (
            f.amd_spectral_fraction[n],
            f.amd_maxpower[n],
            f.amd_maxpower3[n],
            f.amd_frequency1[n],
            f.amd_frequency2[n],
        ...
        (
            ...
            f.amd_maxpower_short[n],
            f.amd_maxpower3_short[n],
            f.amd_frequency1_short[n],
            f.amd_frequency2_short[n],
            f.amd_frequency3_short[n],
    ...
    for n in range(0, clones + 1):
        (
            f.a_minmax_to_mean_density_ratio[n],
            f.a_min_to_max_density_ratio[n],
            f.a_delta_minmax_to_mean_density_ratio[n],
            f.a_delta_min_to_max_density_ratio[n],
        ) = histogram_features(
            a[n], a_min[n], a_max[n], f.a_mean[n], f.a_stddev[n], delta=True
        )

        (
            f.a_minmax_to_mean_density_ratio_short[n],
            f.a_min_to_max_density_ratio_short[n],
            f.a_delta_minmax_to_mean_density_ratio_short[n],
            f.a_delta_min_to_max_density_ratio_short[n],
        ) = histogram_features(
            a_short[n],
            a_min_short[n],
            a_max_short[n],
        ...
        (
            f.e_minmax_to_mean_density_ratio[n],
            f.e_min_to_max_density_ratio[n],
            junk,
            junk,
        ) = histogram_features(
            ec[n], f.e_min[n], f.e_max[n], f.e_mean[n], f.e_stddev[n]
        )
        (
            f.e_minmax_to_mean_density_ratio_short[n],
            f.e_min_to_max_density_ratio_short[n],
            junk,
            junk,
        ) = histogram_features(
            ec_short[n],
        ...
        (
            f.i_minmax_to_mean_density_ratio[n],
            f.i_min_to_max_density_ratio[n],
            junk,
            junk,
        ) = histogram_features(
            inc[n], f.i_min[n], f.i_max[n], f.i_mean[n], f.i_stddev[n]
        )
        (
            f.i_minmax_to_mean_density_ratio_short[n],
            f.i_min_to_max_density_ratio_short[n],
            junk,
            junk,
        ) = histogram_features(
            inc_short,
...
def run_and_MLclassify_TNO(
    ...
    rflag, sim = run_reb.run_simulation(
        sim,
        des=des,
        tmax=tmax,
        tout=50,
    ...
    q_short = a_short * (1.0 - ec_short)

    rflag, apl, ecpl, incpl, nodepl, peripl, mapl, tpl = tools.read_sa_by_hash(
        obj_hash="neptune", archivefile=archivefile, tmin=tmin, tmax=tmax
    )
    if rflag < 1:
        return flag, None, sim

    rflag, xr, yr, zr, vxr, vyr, vzr, tr = tools.calc_rotating_frame(
        des=des,
        planet="neptune",
        archivefile=archivefile,
        clones=clones,
    ...
    tiss_short = apl / a_short + 2.0 * np.cos(inc_short) * np.sqrt(
        a_short / apl * (1.0 - ec_short * ec_short)
    )
    ...
    rflag, a, ec, inc, node, peri, ma, t = tools.read_sa_for_sbody(
        des=des, archivefile=archivefile, clones=clones, tmin=tmin, tmax=tmax
    )
    if rflag < 1:
        return flag, None, sim
    ...
    tiss = apl / a + 2.0 * np.cos(inc) * np.sqrt(a / apl * (1.0 - ec * ec))
    ...
    tno_class.class_probs = classifier.predict_proba(
        tno_class.features.return_features_list()
    )
...
def print_TNO_ML_results(pred_class, classes_dictionary, class_probs, clones=2):
    ...
    for n in range(0, clones + 1):
        print(
            ...
            % (
                n,
                classes_dictionary[pred_class[n]],
                class_probs[n][pred_class[n]],
            ),
            end="",
        )
        for j in range(nclas):
...
def print_TNO_ML_results_to_file(
    des, pred_class, classes_dictionary, class_probs, clones=2
):
...
def histogram_features(x, xmin, xmax, xmean, xstd, delta=False):
    ...
    em_x = (xcounts[0] + xcounts[4]) / (2.0 * xcounts[2])
    ...
    if delta:
        ...
        if xcounts[2] == 0:
            xcounts[2] = 1
        em1 = (xcounts[0] + xcounts[4]) / (2.0 * xcounts[2])
        if xcounts[4] == 0:
            xcounts[4] = 1
        ...
        if xcounts[2] == 0:
            xcounts[2] = 1
        em2 = (xcounts[0] + xcounts[4]) / (2.0 * xcounts[2])
        if xcounts[4] == 0:
            xcounts[4] = 1
        ...
        if xcounts[2] == 0:
            xcounts[2] = 1
        em3 = (xcounts[0] + xcounts[4]) / (2.0 * xcounts[2])
        if xcounts[4] == 0:
            xcounts[4] = 1
        ...
        if xcounts[2] == 0:
            xcounts[2] = 1
        em4 = (xcounts[0] + xcounts[4]) / (2.0 * xcounts[2])
        if xcounts[4] == 0:
            xcounts[4] = 1
...
def rotating_frame_features(rh, phirf):
    ...
    for nr in bins:
        ...
        if rph_count[nr, 0] == 0:
            nadjempty[nr] = 1
            for n in range(1, int(np.floor(nphbin / 2.0)) + 1):
                if rph_count[nr, n] == 0:
                    nadjempty[nr] += 1
                if rph_count[nr, n] != 0:
                    break
            for n in range(int(nphbin) - 1, int(np.floor(nphbin / 2.0)), -1):
                if rph_count[nr, n] == 0:
                    nadjempty[nr] += 1
                if rph_count[nr, n] != 0:
                    break
    ...
    for resorder in range(1, resorder_max + 1):
        rz_peri[resorder] = np.sqrt(
            sinphbar[0, resorder] * sinphbar[0, resorder]
            + cosphbar[0, resorder] * cosphbar[0, resorder]
        )
        rz_apo[resorder] = np.sqrt(
            sinphbar[-1, resorder] * sinphbar[-1, resorder]
...
def read_TNO_training_data(training_file):
    ...
    """
    all_TNOs = pd.read_csv(
        training_file, skipinitialspace=True, index_col=False, low_memory=False
    )
    ...
    filtered_TNOs["simplified_G08"] = filtered_TNOs.apply(
        label_particle_simplifiedG08, axis=1
    )
...
def initialize_TNO_classifier(
    ...
    if not path.exists(classifier_file) or not path.exists(dict_file):
        ...
        clf, score, feature_names, classes_dict = train_and_test_TNO_classifier(
        ...
        classifier, classes_dictionary = train_TNO_classifier(
            training_file=training_file
        )

        sflag = save_TNO_classifier(
            classifier=classifier,
            dictionary=classes_dictionary,
...
def save_TNO_classifier(
    classifier=None, dictionary=None, classifier_file=None, dict_file=None
):
    """
...
def read_trainingset_datafiles(fname):
    ...
    q_sb = a_sb * (1.0 - e_sb)
    tiss_sb = a_pl / a_sb + 2.0 * np.cos(i_sb) * np.sqrt(
        (a_sb / a_pl) * (1.0 - e_sb * e_sb)
    )
    ...
    for j in range(0, lines):
        ...
        [xrf, yrf, zrf, vxrf, vyrf, vzrf] = tools.rotating_frame_cartesian(
            x=x,
            y=y,
            z=z,
            node=node_pl[j],
...
def calc_ML_features_from_trainingset_datafiles(short_fname, long_fname):
    ...
    (
        ...
        phirf_short,
        tn_short,
    ) = read_trainingset_datafiles(short_fname)

    features = calc_ML_features(
        t,
        a,
        e,
        inc,
