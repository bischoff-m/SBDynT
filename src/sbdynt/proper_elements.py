...
default_all_planets_but_mercury_frequencies = (
    "all-planets-minus-mercury-frequencies.pkl"
)
...
def calc_proper_elements(
    ...
    sflag, obj_type, t, clones, a, p, q, h, k = read_sbody_for_proper_elements(
        des=des,
    ...
    if freq_file == None:
        ...
        fflag, planet_freqs = calc_planet_frequencies(
            archivefile, planets, obj_type, tmax=tmax, tmin=tmin
        )
        if fflag:
            try:
    ...
    for j in range(0, clones + 1):
        ...
        summax2hk = np.sum(Yhk_win[gind2 - 3 : gind2 + 4] ** 2.0)
        summax1pq = np.sum(Ypq_win[sind1 - 3 : sind1 + 4] ** 2.0)
        summax2pq = np.sum(Ypq_win[sind2 - 3 : sind2 + 4] ** 2.0)
        ...
        for gi in gindicies:
            ...
            while (
                np.max(
                    [
                        (freq[gi + spreads_g[i]] / freq[gi - spreads_g[i]]),
                        (freq[gi - spreads_g[i]] / freq[gi + spreads_g[i]]),
                    ]
                )
                < spread_dist_g
        ...
        for si in sindicies:
            ...
            while (
                np.max(
                    [
                        (freq[si + spreads_s[i]] / freq[si - spreads_s[i]]),
                        (freq[si - spreads_s[i]] / freq[si + spreads_s[i]]),
                    ]
                )
                < spread_dist_s
        ...
        for gi in gindicies:
            ...
            if gi == gind or abs(gi - gind) < freq_dist_lims_g[i]:
                ...
                continue
            if spreads_g[i] > 0:
                if (gi - 2 * spreads_g[i] - 1) <= 0 or freq[
                    max(gi - 2 * spreads_g[i] - 1, 1)
                ] * freq[gi] < 0:
                    # filtered based on average of just the higher frequency side
                    tmpY = Yhk_f[
                        gi + spreads_g[i] + 1 : gi + 2 * spreads_g[i] + 1
                    ].copy()
                elif (gi + 2 * spreads_g[i] + 1) >= len(freq) or freq[
                    min(gi + 2 * spreads_g[i] + 1, len(freq) - 1)
                ] * freq[gi] < 0:
        ...
        for si in sindicies:
            ...
            if si == sind or abs(si - sind) < freq_dist_lims_s[i]:
                ...
                continue
            if spreads_s[i] > 0:
                if (si - 2 * spreads_s[i] - 1) <= 0 or freq[
                    max(si - 2 * spreads_s[i] - 1, 1)
                ] * freq[si] < 0:
                    # filtered based on average of just the higher frequency side
                    tmpY = Ypq_f[
                        si + spreads_s[i] + 1 : si + 2 * spreads_s[i] + 1
                    ].copy()
                elif (si + 2 * spreads_s[i] + 1) >= len(freq) or freq[
                    min(si + 2 * spreads_s[i] + 1, len(freq) - 1)
                ] * freq[si] < 0:
                    # filtered based on average of just the higher negative frequency side
                    tmpY = Ypq_f[
...
def calc_planet_frequencies(
    ...
    for pl in ordered_planets:
        if obj_type == "tno":
            ...
            tmin = 0
            tmax = 0.5e6
            flag, at, et, inct, nodet, aperit, mat, tt = tools.read_sa_by_hash(
            ...
            tmin = 0.0
            tmax = 5e6
            flag, at, ep, incp, nodep, aperip, mat, tp = tools.read_sa_by_hash(
                obj_hash=pl, archivefile=archivefile, tmin=tmin, tmax=tmax
            )
            dt = tp[1] - tp[0]
            nout = len(tp)
        ...
        ifmax = np.argmax(np.abs(Yh[1:]) ** 2.0) + 1
        ...
        if pl != "jupiter":
            ...
            ifmax = np.argmax(np.abs(Yp[1:]) ** 2.0) + 1
    ...
    if planet_freqs.g5 and planet_freqs.g6:
        z1 = 2.0 * planet_freqs.g6 - planet_freqs.g5
        z2 = 2.0 * planet_freqs.g5 - planet_freqs.g6
...
def calc_filter_frequencies(planet_freqs, g, s):
    ...
    z[36] = g + g5 - s7 - s6
    z[37] = g + g6 - s7 - s6
    z[38] = s - 2.0 * s7 + s6  # sonly
    z[39] = s + s7 - 2.0 * s6  # sonly
    z[40] = 2.0 * s - s7 - s6  # sonly
    z[41] = s + g5 - g6 - s7
    z[42] = s - g5 + g6 - s7
    z[43] = s + g5 - g6 - s6
    z[44] = s - g5 + g6 - s6
    ...
    z[52] = s - g5 - g6 + s7
    z[53] = s - g5 - g6 + s6
    z[54] = 2.0 * g - 2.0 * g5  # gonly
    z[55] = 2.0 * g - 2.0 * g6  # gonly
    z[56] = 2.0 * s - 2.0 * s7  # sonly
    z[57] = 2.0 * s - 2.0 * s6  # sonly
    z[58] = g - 2.0 * g6 + g7  # gonly
    z[59] = g - 3.0 * g6 + 2.0 * g5  # gonly
    z[60] = 2.0 * (g - g6) + (s - s6)
    ...
    z[65] = g - (2.0 * (g - g6) + s - s6)
    z[66] = g - (3.0 * (g - g6) + s - s6)
    z[67] = g - ((g - g6) + s - s6)
    z[68] = s - (2.0 * (g - g6) + s - s6)
    z[69] = s - (3.0 * (g - g6) + s - s6)
    z[70] = s - ((g - g6) + s - s6)
    z[71] = g - 2.0 * g7 + g6  # gonly
    z[72] = -4.0 * g + 4.0 * g7  # gonly
    z[73] = -2 * s - s6  # sonly
    z[74] = -g + 2 * s - g5
    ...
    z[81] = -g + s + g6 - s8
    z[82] = -g + s + g7 - s8
    z[83] = 2.0 * g - 2.0 * g8  # gonly
    z[84] = 2.0 * g - 2.0 * g7  # gonly
    z[85] = 2.0 * g - 2.0 * s8
    z[86] = 2.0 * s - 2.0 * g7
    z[87] = g - 2.0 * g7  # gonly
    z[88] = g - 2.0 * g8  # gonly
    z[89] = s - 2.0 * s7  # sonly
    z[90] = s - 2.0 * s8  # sonly
    z[91] = 3.0 * g - 2.0 * g8  # gonly
    z[92] = 2.0 * g - g7  # gonly
    z[93] = 2.0 * g - g8  # gonly
    z[94] = 2.0 * s - 2.0 * s7  # sonly
    z[95] = 2.0 * s - 2.0 * s8  # sonly
    z[96] = 3.0 * g - (g5 - g7)  # gonly
    z[97] = g5 - g7  # gonly
    z[98] = 2.0 * g - (g5 - g7)  # gonly
    z[99] = g - (g5 - g7 - g8)
    z[100] = -g + (g5 - g7 - g8)
    z[101] = -s + (g5 - g7 - g8)
